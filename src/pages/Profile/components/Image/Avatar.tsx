...
const ProfileAvatar: React.FunctionComponent<IProfileAvatarProps> = (props) => {
  const { access, url, onChangeImage } = props;
  ...
  return (
    <div className="w-24 h-24 font-roboto rounded-full inline-block overflow-hidden absolute -translate-y-1/2 left-2 border-2 border-dark-smooth-on-surface group">
      <img src={url ? url : GuestProfile} alt="" referrerPolicy="no-referrer" className="object-cover w-full h-full" />

      {access && (
        <React.Fragment>
          <input onChange={onChangeImage} type={'file'} id={'upload-avatar'} accept={'image/*'} className="hidden" />
          <label
